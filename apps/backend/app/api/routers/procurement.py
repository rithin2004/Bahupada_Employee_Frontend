...
async def get_purchase_bill(purchase_bill_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
        ...
        item_rows.append(
            {
                ...
                "expiry_date": str(item.expiry_date) if item.expiry_date else "",
                "quantity": str(item.quantity or 0),
                "quantity_1st": str(item.quantity_1st or ""),
                "quantity_2nd": str(item.quantity_2nd or ""),
                "quantity_3rd": str(item.quantity_3rd or ""),
                "base_quantity": str(item.base_quantity or 0),
                "damaged_quantity": str(item.damaged_quantity or 0),
                "unit_price": str(item.unit_price or 0),
                "rate_value": str(item.rate_value or item.unit_price or 0),
                "rate_unit_level": item.rate_unit_level or 1,
                ...
                "line_tax_amount": str(item.line_tax_amount or 0),
                "line_total_amount": str(item.line_total_amount or 0),
            }
        )
    ...
    return {
        ...
        "bill_number": bill.bill_number,
        "bill_date": str(bill.bill_date),
        "entry_number": bill.entry_number,
        "received_date": str(bill.received_date) if bill.received_date else None,
        ...
        "challan_reference_no": challan.reference_no if challan else "",
        "entry_mode": "challan" if challan else "direct",
        "freight_amount": str(bill.freight_amount or 0),
        "notes": bill.notes or "",
        "subtotal": str(bill.subtotal or 0),
        "gst_amount": str(bill.gst_amount or 0),
        "total_amount": str(bill.total_amount or 0),
        "items": item_rows,
    }

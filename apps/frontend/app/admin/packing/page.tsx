...
export default async function AdminPackingPage() {
  ...
  return (
    <AppShell role="admin" activeKey="packing" userName="Admin User">
      <div className="space-y-6">
        <div className="space-y-2">
          <LiveBadge live={data.live} />
          <h2 className="text-2xl font-semibold tracking-tight">Packing</h2>
          <p className="text-sm text-muted-foreground">Ready-to-dispatch tasks and packaging quality indicators.</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <MetricCard
            label="Labeled Packs"
            value={data.items.filter((item) => Boolean(item.pack_label)).length}
          />
          <MetricCard

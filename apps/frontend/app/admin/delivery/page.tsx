...
async function getDeliveryData() {
  try {
    ...
    if (!warehouse?.id) {
      return { warehouseName: "-", count: 0, items: [], live: false };
    }
...
export default async function AdminDeliveryPage() {
  ...
  return (
    <AppShell role="admin" activeKey="delivery" userName="Admin User">
      <div className="space-y-6">
        <div className="space-y-2">
          <LiveBadge live={data.live} />
          <h2 className="text-2xl font-semibold tracking-tight">Delivery</h2>
          <p className="text-sm text-muted-foreground">Dispatch-ready delivery workload from routing endpoints.</p>
        </div>

        <div className="grid gap-4 md:grid-cols-3">
          <MetricCard label="Warehouse" value={data.warehouseName} />
          <MetricCard label="Ready Items" value={data.count} />
